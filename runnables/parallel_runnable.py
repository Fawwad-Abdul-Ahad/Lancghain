...
prompt1 = PromptTemplate(
    template = 'Generate a 5 line summary of the following topic {topic}',
    input_variables=["topic"]
)

prompt2 = PromptTemplate(
    template = 'Generate the quiz of the following topic {topic}',
    input_variables=['topic']
)

prompt3 = PromptTemplate(
    template = 'Merge the whole five line summary and quiz /n summary -> {summary} ans quiz -> {quiz}',
    input_ariables = ['summary', 'quiz']
)

parallel_chain = RunnableParallel({
    'summary': RunnableSequence( prompt1, model1, parser),
    'quiz': RunnableSequence (prompt2, model1, parser),
})
